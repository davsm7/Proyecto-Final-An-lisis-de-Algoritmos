...
# =============================
# ALGORITMO VORAZ: HUFFMAN
# =============================

class NodoHuffman:
    """Nodo para el árbol de Huffman"""
    def __init__(self, pais, frecuencia):
        self.pais = pais
...
def construir_arbol_huffman(frecuencias_paises):
    """
    Construye un árbol de Huffman basado en frecuencias de infectados.
    Algoritmo voraz: siempre une los dos nodos de menor frecuencia.
    """
    if not frecuencias_paises:
        return None
    
    # Crear heap con nodos hoja
    heap = [NodoHuffman(pais, freq) for pais, freq in frecuencias_paises.items() if freq > 0]
    heapq.heapify(heap)
    
    # Construir árbol combinando nodos (greedy: menor frecuencia primero)
    while len(heap) > 1:
...
def generar_codigos_huffman(nodo, codigo_actual="", codigos=None):
    """
    Genera códigos binarios para cada país usando el árbol de Huffman.
    Países con más infectados obtienen códigos más cortos (voraz).
    """
    if codigos is None:
        codigos = {}
    ...
    if nodo.pais is not None:
        codigos[nodo.pais] = codigo_actual if codigo_actual else "0"
        return codigos
    
    # Recorrer árbol: 0 = izquierda, 1 = derecha
...
def comprimir_datos_huffman(datos_infectados, dia):
    """
    Comprime datos de infectados usando Huffman.
    Retorna: códigos, árbol, tasa de compresión
    """
    # Obtener frecuencias (infectados en el día dado)
    frecuencias = {pais: int(datos_infectados[pais][dia]) 
                   for pais in datos_infectados.keys()}
    ...
    # Calcular bits sin compresión (asumiendo 8 bits por país)
    bits_originales = len([p for p, f in frecuencias.items() if f > 0]) * 8
    
    # Calcular bits con Huffman
    bits_comprimidos = sum(len(codigos[p]) for p in codigos.keys())
    
    # Tasa de compresión
    tasa = ((bits_originales - bits_comprimidos) / bits_originales * 100) if bits_originales > 0 else 0
    
    return codigos, arbol, bits_originales, bits_comprimidos, tasa

# =============================
# ALGORITMO VORAZ: RUTA DE CONTAGIO
# =============================

def predecir_ruta_contagio_voraz(pais_origen, max_pasos=10):
    """
    Algoritmo voraz: predice la ruta de contagio eligiendo 
    siempre el vecino MÁS VULNERABLE (sistema precario + mayor población)
    """
    ruta = [pais_origen]
    ...
    print(f"\n{'='*70}")
    print(f"🎯 ALGORITMO VORAZ: PREDICCIÓN DE RUTA DE CONTAGIO")
    print(f"País origen: {pais_origen}")
    print(f"{'='*70}\n")
    
    for paso in range(max_pasos):
        if actual not in vecinos:
            print(f"❌ {actual} no tiene vecinos definidos")
            break
        
        # Greedy Choice: elegir vecino más vulnerable
        mejor_vecino = None
        mejor_score = -1
        candidatos = []
        
        for vecino in vecinos[actual]:
            if vecino not in visitados and vecino in paises:
                config = config_paises[vecino]
                
                # Score de vulnerabilidad (criterio voraz)
                score = config["poblacion"] / 1000000  # Población base
                if config["sistema"] == "precario":
                    score *= 3
                elif config["sistema"] == "normal":
                    score *= 1.5
                
                candidatos.append((vecino, score, config["sistema"]))
        ...
        if mejor_vecino is None:
            print(f"✅ No hay más vecinos vulnerables desde {actual}")
            break
        
        # Mostrar decisión voraz
        print(f"Paso {paso + 1}: Desde {actual}")
        print(f"  Candidatos evaluados:")
        for cand, sc, sist in sorted(candidatos, key=lambda x: x[1], reverse=True):
            emoji = "🏆" if cand == mejor_vecino else "  "
            print(f"    {emoji} {cand:20s} | Score: {sc:6.2f} | Sistema: {sist}")
        print(f"  ➡️ ELECCIÓN VORAZ: {mejor_vecino} (score: {mejor_score:.2f})\n")
    ...
    print(f"{'='*70}")
    print(f"🛣️ RUTA PREDICHA: {' → '.join(ruta)}")
    print(f"Total de pasos: {len(ruta) - 1}")
    print(f"{'='*70}\n")
...
sistemas_sanitarios = {
    ...
    "Czechia": "normal", "Slovenia": "normal", "Estonia": "normal", "Poland": "normal",
...
PARAMS_SISTEMA = {
    "desarrollado": {
        ...
        "gamma_post": 0.10,
        "mu": 0.00015,
        "dia_recuperacion": 235,
        "vac_inicio": 250,
        "vac_rate": 0.010,
        "umbral_contagiar": 20000,
        "umbral_ser_contagiado": 20000,
    },
    "normal": {
        "beta": 0.15,
        "gamma": 0.00,
        ...
        "vac_inicio": 240,
        "vac_rate": 0.007,
        "umbral_contagiar": 5000,
        "umbral_ser_contagiado": 5000,
    },
    "precario": {
        "beta": 0.3,
        "gamma": 0.00,
        ...
        "vac_inicio": 250,
        "vac_rate": 0.003,
        "umbral_contagiar": 1000,
        "umbral_ser_contagiado": 1000,
    }
}
...
for pais in paises:
    ...
    config_paises[pais] = {
        ...
        "umbral_contagiar": params["umbral_contagiar"],
        "umbral_ser_contagiado": params["umbral_ser_contagiado"],
    }

dias = 300
dt = 1.0
TASA_CONTAGIO_BASE = 0.90
INFECCION_INICIAL = 30
...
vecinos = {
    ...
    "Germany": ["France", "Poland", "Czechia", "Austria", "Switzerland", "Belgium", "Netherlands", "Denmark"],
    "Portugal": ["Spain"],
    "United Kingdom": ["Ireland"],
    "Ireland": ["United Kingdom"],
    "Belgium": ["France", "Germany", "Netherlands"],
    "Netherlands": ["Belgium", "Germany"],
    "Poland": ["Germany", "Czechia", "Slovakia", "Ukraine", "Belarus", "Lithuania"],
    "Czechia": ["Germany", "Poland", "Slovakia", "Austria"],
    "Austria": ["Germany", "Czechia Rep.", "Slovakia", "Hungary", "Slovenia", "Italy", "Switzerland"],
...
def iniciar_simulacion(pais_inicial):
    ...
    print(f"\n{'='*70}")
    print(f"INICIANDO SIMULACIÓN - MODELO SIRD CON UMBRALES PERSONALIZADOS")
    ...
    print(f"Umbral de resistencia: {config_paises[pais_inicial]['umbral_ser_contagiado']:,}")
    print(f"")
    print(f"UMBRALES POR SISTEMA:")
    print(f"🟢 Desarrollado: contagia a 15k | se infecta a 40k")
    print(f"🟡 Normal:        contagia a 10k | se infecta a 20k")
    print(f"🔴 Precario:      contagia a  5k | se infecta a 10k")
    print(f"{'='*70}\n")
    ...
    for pais in paises:
        ...
        N = config["poblacion"]
        datos_I[pais] = [0 for _ in range(dias)]
        datos_S[pais] = [N for _ in range(dias)]
        ...
        datos_R[pais] = [0 for _ in range(dias)]
        datos_D[pais] = [0 for _ in range(dias)]
        datos_V[pais] = [0 for _ in range(dias)]
    ...
    for dia in range(1, dias):
        # FASE 1: Verificar contagios ANTES de actualizar ecuaciones
        nuevos_contagios = []
        
        for pais in paises:
            if pais not in paises_infectados:
                continue
                
            config = config_paises[pais]
            
            # Contagio a países vecinos
            if datos_I[pais][dia - 1] >= config["umbral_contagiar"] and pais in vecinos:
                for vecino in vecinos[pais]:
                    if vecino in paises and vecino not in paises_infectados:
                        config_vecino = config_paises[vecino]
                        
                        if datos_I[pais][dia - 1] >= config_vecino["umbral_ser_contagiado"]:
                            ...
                            if np.random.random() < prob_contagio:
                                nuevos_contagios.append((pais, vecino))
                                print(pais, vecino)
        
        # Aplicar contagios
        print(nuevos_contagios)
        for pais_origen, vecino in nuevos_contagios:
            print(vecino)
            if vecino not in paises_infectados:
                paises_infectados.add(vecino)
                config_vecino = config_paises[vecino]
                N_vecino = config_vecino["poblacion"]
                datos_I[vecino][dia] = INFECCION_INICIAL
                datos_S[vecino][dia] = N_vecino - INFECCION_INICIAL
                datos_R[vecino][dia] = 0
                datos_D[vecino][dia] = 0
                datos_V[vecino][dia] = 0
                
                print(f"Día {dia}: {pais_origen} ({int(datos_I[pais_origen][dia-1]):,} inf.) → {vecino} "
                    f"[{config_paises[vecino]['sistema']}] (inicializado con {INFECCION_INICIAL} inf.)")
        ...
        # FASE 2: Actualizar ecuaciones diferenciales
        for pais in paises:
            if pais in [vecino for _, vecino in nuevos_contagios]:
                continue
            ...
            # Estado actual
            S = max(0, min(datos_S[pais][dia - 1], N-INFECCION_INICIAL))
            I = max(INFECCION_INICIAL, datos_I[pais][dia - 1])
            ...
            # Tasa de recuperación variable
            if dia < config["dia_recuperacion"]:
                gamma = 0.0
            else:
                gamma = config["gamma_post"]
            ...
            if dia%10 == 0:
                print(f'Pais: {pais}, dia: {dia}, beta: {beta}, S: {S}, I: {I}, N: {N}')
                    ...
                    for vecino in vecinos[pais]:
                        if vecino in paises and vecino not in paises_infectados:
                            ...
                            if datos_I[pais][dia] >= config_vecino["umbral_ser_contagiado"]:
                                ...
                                if np.random.random() < prob_contagio:
                                    paises_infectados.add(vecino)
                                    S_vecino = datos_S[vecino][dia - 1] if dia > 0 else config_paises[vecino]["poblacion"]
                                    casos_importados = min(INFECCION_INICIAL, S_vecino)
                                    
                                    if casos_importados < INFECCION_INICIAL:
                                        casos_importados = INFECCION_INICIAL
                                    
                                    datos_I[vecino][dia] = max(datos_I[vecino][dia], casos_importados)
                                    datos_S[vecino][dia] = max(0, datos_S[vecino][dia] - casos_importados)
                                    
                                    print(f"Día {dia}: {pais} ({int(datos_I[pais][dia]):,} inf.) → {vecino} "
                                        f"[{config_vecino['sistema']}] ({int(casos_importados)} inf. iniciales)")
...
class SimuladorPandemia:
    def __init__(self, root):
        self.root = root
        self.root.title("Simulador de Pandemia COVID-19 - Modelo SIRD con Algoritmos Voraces")
        ...
        # ===== SECCIÓN ALGORITMOS VORACES =====
        tk.Label(self.frame_izquierdo, text="🎯 Algoritmos Voraces (Greedy)", 
                font=("Arial", 12, "bold"), fg="#FF6600").pack(pady=5)
        
        frame_botones_voraz = tk.Frame(self.frame_izquierdo)
        frame_botones_voraz.pack(pady=5, fill="x", padx=10)
        
        tk.Button(frame_botones_voraz, text="🛣️ Predecir Ruta de Contagio", 
                command=self.predecir_ruta_voraz, font=("Arial", 10), 
                bg="#FF9800", fg="white").pack(fill="x", pady=2)
        tk.Button(frame_botones_voraz, text="🗜️ Compresión Huffman (Día Actual)", 
                command=self.comprimir_huffman, font=("Arial", 10),
                bg="#9C27B0", fg="white").pack(fill="x", pady=2)
    ...
    def iniciar_simulacion(self):
        ...
        config = config_paises[pais_inicial]
        messagebox.showinfo("✅ Simulación Iniciada", 
                        f"País de origen: {pais_inicial}\n"
                        f"Sistema: {config['sistema'].capitalize()}\n"
                        f"Población: {config['poblacion']:,}\n\n"
                        f"UMBRALES:\n"
                        f"Contagia a vecinos: {config['umbral_contagiar']:,} infectados\n"
                        f"Se infecta cuando vecino tiene: {config['umbral_ser_contagiado']:,}")
    
    # ===== MÉTODOS DE ALGORITMOS VORACES =====
    
    def predecir_ruta_voraz(self):
        """Ejecuta el algoritmo voraz de predicción de ruta"""
        if self.datos_infectados is None:
            messagebox.showwarning("Advertencia", "Inicia la simulación primero")
            return
        
        pais_inicial = self.combo_pais.get()
        ruta = predecir_ruta_contagio_voraz(pais_inicial, max_pasos=15)
        
        # Crear ventana con resultados
        ventana_ruta = tk.Toplevel(self.root)
        ventana_ruta.title("🎯 Algoritmo Voraz: Predicción de Ruta")
        ventana_ruta.geometry("700x600")
        
        # Título
        tk.Label(ventana_ruta, text="🛣️ RUTA PREDICHA DE CONTAGIO", 
                font=("Arial", 14, "bold"), fg="#FF6600").pack(pady=10)
        
        tk.Label(ventana_ruta, text=f"Origen: {pais_inicial}", 
                font=("Arial", 11)).pack(pady=5)
        
        # Frame con scroll para la ruta
        frame_scroll = tk.Frame(ventana_ruta)
        frame_scroll.pack(fill="both", expand=True, padx=20, pady=10)
        
        scrollbar = ttk.Scrollbar(frame_scroll)
        scrollbar.pack(side="right", fill="y")
        
        text_ruta = tk.Text(frame_scroll, font=("Courier", 10), 
                           yscrollcommand=scrollbar.set, wrap="word")
        text_ruta.pack(side="left", fill="both", expand=True)
        scrollbar.config(command=text_ruta.yview)
        
        # Construir texto de la ruta
        texto = f"{'='*60}\n"
        texto += f"ALGORITMO VORAZ: Selección del vecino MÁS VULNERABLE\n"
        texto += f"{'='*60}\n\n"
        texto += f"Criterio: Sistema precario (x3) > Normal (x1.5) > Desarrollado (x1)\n"
        texto += f"         + Población más grande\n\n"
        texto += f"{'='*60}\n"
        texto += f"RUTA PREDICHA ({len(ruta)} pasos):\n"
        texto += f"{'='*60}\n\n"
        
        for i, pais in enumerate(ruta):
            config = config_paises[pais]
            emoji = "🟢" if config["sistema"] == "desarrollado" else \
                    ("🟡" if config["sistema"] == "normal" else "🔴")
            
            if i == 0:
                texto += f"🏁 INICIO: {pais}\n"
            else:
                texto += f"   ⬇️\n"
                texto += f"{i}. {emoji} {pais}\n"
            
            texto += f"   Sistema: {config['sistema'].capitalize()}\n"
            texto += f"   Población: {config['poblacion']:,}\n"
            texto += f"   Umbral contagio: {config['umbral_contagiar']:,}\n\n"
        
        texto += f"{'='*60}\n"
        texto += f"✅ Predicción completada\n"
        texto += f"Total de países en la ruta: {len(ruta)}\n"
        texto += f"{'='*60}\n"
        
        text_ruta.insert("1.0", texto)
        text_ruta.config(state="disabled")
        
        # Botón cerrar
        tk.Button(ventana_ruta, text="Cerrar", command=ventana_ruta.destroy,
                 font=("Arial", 11), bg="#f44336", fg="white").pack(pady=10)
    
    def comprimir_huffman(self):
        """Ejecuta el algoritmo de compresión Huffman"""
        if self.datos_infectados is None:
            messagebox.showwarning("Advertencia", "Inicia la simulación primero")
            return
        
        dia = self.dia_simulacion
        codigos, arbol, bits_orig, bits_comp, tasa = comprimir_datos_huffman(self.datos_infectados, dia)
        
        if not codigos:
            messagebox.showinfo("Huffman", "No hay datos de infectados para comprimir en este día")
            return
        
        # Crear ventana con resultados
        ventana_huffman = tk.Toplevel(self.root)
        ventana_huffman.title("🗜️ Algoritmo Huffman: Compresión de Datos")
        ventana_huffman.geometry("800x700")
        
        # Título
        tk.Label(ventana_huffman, text="🗜️ COMPRESIÓN HUFFMAN", 
                font=("Arial", 14, "bold"), fg="#9C27B0").pack(pady=10)
        
        tk.Label(ventana_huffman, text=f"Día de simulación: {dia + 1}", 
                font=("Arial", 11)).pack(pady=5)
        
        # Estadísticas de compresión
        frame_stats = tk.Frame(ventana_huffman, bg="#f0f0f0", relief="ridge", bd=2)
        frame_stats.pack(fill="x", padx=20, pady=10)
        
        tk.Label(frame_stats, text=f"📊 Bits originales: {bits_orig}", 
                font=("Arial", 11), bg="#f0f0f0").pack(anchor="w", padx=10, pady=3)
        tk.Label(frame_stats, text=f"📦 Bits comprimidos: {bits_comp}", 
                font=("Arial", 11), bg="#f0f0f0").pack(anchor="w", padx=10, pady=3)
        tk.Label(frame_stats, text=f"💾 Tasa de compresión: {tasa:.2f}%", 
                font=("Arial", 11, "bold"), bg="#f0f0f0", fg="#4CAF50").pack(anchor="w", padx=10, pady=3)
        
        # Tabla de códigos
        tk.Label(ventana_huffman, text="📋 Códigos Huffman por País", 
                font=("Arial", 12, "bold")).pack(pady=10)
        
        frame_tabla = tk.Frame(ventana_huffman)
        frame_tabla.pack(fill="both", expand=True, padx=20, pady=10)
        
        scrollbar_h = ttk.Scrollbar(frame_tabla, orient="vertical")
        scrollbar_h.pack(side="right", fill="y")
        
        tabla_huffman = ttk.Treeview(frame_tabla, 
                                     columns=("Pais", "Infectados", "Codigo", "Bits"), 
                                     show="headings", height=15,
                                     yscrollcommand=scrollbar_h.set)
        tabla_huffman.heading("Pais", text="País")
        tabla_huffman.heading("Infectados", text="Infectados")
        tabla_huffman.heading("Codigo", text="Código Huffman")
        tabla_huffman.heading("Bits", text="Longitud")
        
        tabla_huffman.column("Pais", width=150)
        tabla_huffman.column("Infectados", width=120, anchor="center")
        tabla_huffman.column("Codigo", width=200, anchor="center")
        tabla_huffman.column("Bits", width=80, anchor="center")
        
        tabla_huffman.pack(side="left", fill="both", expand=True)
        scrollbar_h.config(command=tabla_huffman.yview)
        
        # Ordenar países por frecuencia (descendente)
        paises_ordenados = sorted(codigos.items(), 
                                  key=lambda x: self.datos_infectados[x[0]][dia], 
                                  reverse=True)
        
        for pais, codigo in paises_ordenados:
            infectados = int(self.datos_infectados[pais][dia])
            tabla_huffman.insert("", "end", 
                               values=(pais, f"{infectados:,}", codigo, len(codigo)))
        
        # Explicación del algoritmo
        frame_explicacion = tk.Frame(ventana_huffman, bg="#fffbea", relief="ridge", bd=2)
        frame_explicacion.pack(fill="x", padx=20, pady=10)
        
        explicacion = (
            "🎯 ALGORITMO VORAZ:\n"
            "1. Construye árbol binario uniendo nodos de menor frecuencia\n"
            "2. Países con MÁS infectados → códigos MÁS CORTOS\n"
            "3. Países con MENOS infectados → códigos MÁS LARGOS\n"
            "4. Resultado: compresión óptima de datos"
        )
        
        tk.Label(frame_explicacion, text=explicacion, 
                font=("Arial", 10), bg="#fffbea", justify="left").pack(padx=10, pady=10)
        
        # Botón cerrar
        tk.Button(ventana_huffman, text="Cerrar", command=ventana_huffman.destroy,
                 font=("Arial", 11), bg="#f44336", fg="white").pack(pady=10)
    
    # ===== MÉTODOS DE ANÁLISIS (DIVIDE Y VENCERÁS) =====
    ...
    def actualizar_mapa(self, inicial=False):
        ...
        for pais in paises:
            ...
            if inf > 0 or mue > 0 or rec > 0:
                ...
                if inf > 0 and self.dia_simulacion % 10 == 0:
                    print(f"DEBUG Día {self.dia_simulacion}: {pais} tiene {inf} infectados")

        self.tabla.insert("", "end", values=(f"═══ TOTAL ({paises_mostrados} países) ═══", "", total_inf, total_mue, total_rec))
    ...
    def simular_tiempo(self):
        ...
        if not self.pausado:
            ...
            if self.dia_simulacion >= 235 and self.dia_simulacion <= 250:
                if self.dia_simulacion == 235:
                    self.titulo.config(text="🏥 ¡FASE DE RECUPERACIÓN INICIADA! 🏥", fg="green")
                elif self.dia_simulacion == 250:
